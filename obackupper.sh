...
Commands:
  ...
  list                Show hostname folders, then backups; selected backup can be restored/deleted
  place               Reconfigure backup storage; final folder remains $BACKUP_ROOT_DIRNAME
...
prompt_value() {
    local text="$1" def="${2:-}" ans prompt
    [ -n "$def" ] && prompt="$text [$def]: " || prompt="$text: "
...
ask_yes_no() { local a; a=$(prompt_value "$1" "${2:-n}"); a=$(printf "%s" "$a" | tr A-Z a-z); [ "$a" = y ] || [ "$a" = yes ]; }
...
normalize_backup_root_path() { local p="${1:-$BACKUP_ROOT}"; [ -n "$p" ] || p="$FALLBACK_BACKUP_ROOT"; p=${p%/}; case "$p" in */$BACKUP_ROOT_DIRNAME) echo "$p";; *) echo "$p/$BACKUP_ROOT_DIRNAME";; esac; }
normalize_restore_target() { local t="${1:-$DEFAULT_RESTORE_TARGET}"; [ "$t" = / ] && echo "$DEFAULT_RESTORE_TARGET" || echo "${t%/}"; }
...
remove_installed_artifacts() { local p any=0; for p in "$INSTALL_PATH" "$LEGACY_INSTALL_PATH" "$SHORTCUT_PATH" "$CONFIG_PATH"; do if [ -e "$p" ] || [ -L "$p" ]; then rm -f "$p"; echo "Removed: $p"; any=1; else echo "Not present: $p"; fi; done; [ "$any" = 1 ] || echo "Nothing to remove."; }
...
release_lock() { [ -d "$LOCK_DIR" ] || return 0; local pid; pid=$(cat "$LOCK_DIR/pid" 2>/dev/null || true); [ -n "$pid" ] && [ "$pid" != $$ ] && return 0; rm -rf "$LOCK_DIR"; }
...
owrt_value() { local key="$1"; [ -r /etc/openwrt_release ] && . /etc/openwrt_release; eval "printf '%s' \"\${$key:-}\""; }
...
append_section() { local s="$1"; shift; echo; echo "[$s]"; "$@" 2>&1 || true; }
write_metadata() {
    local dir="$1" recorded="$2" pm cnt
    pm=$(detect_package_manager); cnt=$(wc -l < "$dir/$PACKAGE_LIST_NAME" | tr -d ' ')
...
backup_name_date_time() {
    local dir="$1" leaf name created date time
    leaf=$(basename "$dir"); name=$(backup_identity_value "$dir" openwrt_description || true); created=$(backup_identity_value "$dir" backup_created || true)
    if [ -n "$created" ]; then set -- $created; date="${1:-unknown}"; time="${2:-unknown}"; [ -n "$name" ] || name="$leaf"; echo "$name|$date|$time"; return; fi
    date=$(printf "%s" "$leaf" | sed -n 's/^.*_\([0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]\)_[0-9-]*$/\1/p'); time=$(printf "%s" "$leaf" | sed -n 's/^.*_[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]_\([0-9-]*\)$/\1/p' | tr - :)
    [ -n "$date" ] || date=unknown; [ -n "$time" ] || time=unknown; [ -n "$name" ] || name=$(printf "%s" "$leaf" | sed 's/_[0-9][0-9][0-9][0-9]-[0-9][0-9]-[0-9][0-9]_[0-9-]*$//'); echo "$name|$date|$time"
}
...
gather_backup_candidates() {
    local host_root="$1" out="$2" sortf="/tmp/${SCRIPT_BASENAME}.backup-sorted.$$" b sort ndt name date time size checks
...
gather_host_candidates() {
    local root="$1" out="$2" sortf="/tmp/${SCRIPT_BASENAME}.host-sorted.$$" tmp="/tmp/${SCRIPT_BASENAME}.hosts.$$" cur h key safe hf cnt latest date time pref psort
    : > "$out"; : > "$sortf"; rm -rf "$tmp"; mkdir -p "$tmp"; cur=$(current_hostname)
...
print_host_table() { local f="$1" i=1 line name cnt date time pref note; printf "  %-3s %-24s %-8s %-12s %-10s %-8s\n" No Hostname Backups "Last date" "Last time" Priority; printf "  %-3s %-24s %-8s %-12s %-10s %-8s\n" -- -------- ------- --------- --------- --------; while IFS= read -r line; do name=$(echo "$line"|cut -d'|' -f2); cnt=$(echo "$line"|cut -d'|' -f3); date=$(echo "$line"|cut -d'|' -f4); time=$(echo "$line"|cut -d'|' -f5); pref=$(echo "$line"|cut -d'|' -f6); note=; [ "$pref" = 1 ] && note=current; if [ "$pref" = 1 ]; then printf "%s  %-3s %-24s %-8s %-12s %-10s %-8s%s\n" "$C_GREEN" "$i" "$name" "$cnt" "$date" "$time" "$note" "$C_RESET"; else printf "  %-3s %-24s %-8s %-12s %-10s %-8s\n" "$i" "$name" "$cnt" "$date" "$time" "$note"; fi; i=$((i+1)); done < "$f"; }
print_backup_table() { local f="$1" i=1 line name date time size checks; printf "  %-3s %-34s %-12s %-10s %-10s %-8s\n" No "OpenWrt version" Date Time Size Checks; printf "  %-3s %-34s %-12s %-10s %-10s %-8s\n" -- --------------- ---- ---- ---- ------; while IFS= read -r line; do name=$(echo "$line"|cut -d'|' -f2); date=$(echo "$line"|cut -d'|' -f3); time=$(echo "$line"|cut -d'|' -f4); size=$(echo "$line"|cut -d'|' -f5); checks=$(echo "$line"|cut -d'|' -f6); printf "  %-3s %-34s %-12s %-10s %-10s %-8s\n" "$i" "$name" "$date" "$time" "$size" "$checks"; i=$((i+1)); done < "$f"; }
number_in_range() { case "$1" in ''|*[!0-9]*) return 1;; esac; [ "$1" -ge 1 ] && [ "$1" -le "$2" ]; }

select_host_candidate() { local f="$1" total sel; total=$(wc -l < "$f"|tr -d ' '); SELECTED_HOST_DIR=; echo "Hostname groups in: $BACKUP_ROOT"; echo; print_host_table "$f"; echo; is_interactive || return 1; while true; do sel=$(prompt_value "Enter hostname number or q to cancel" 1); case "$sel" in q|Q) echo Cancelled.; return 1;; esac; if number_in_range "$sel" "$total"; then SELECTED_HOST_DIR=$(sed -n "${sel}p" "$f"|cut -d'|' -f1); return; fi; echo "Enter a number from 1 to $total, or q."; done; }
select_backup_candidate() { local f="$1" host="$2" total sel; total=$(wc -l < "$f"|tr -d ' '); SELECTED_BACKUP_DIR=; echo "Backups for hostname: $(basename "$host")"; echo; print_backup_table "$f"; echo; is_interactive || return 1; while true; do sel=$(prompt_value "Enter backup number or q to cancel" 1); case "$sel" in q|Q) echo Cancelled.; return 1;; esac; if number_in_range "$sel" "$total"; then SELECTED_BACKUP_DIR=$(sed -n "${sel}p" "$f"|cut -d'|' -f1); return; fi; echo "Enter a number from 1 to $total, or q."; done; }

print_backup_identity_lines() { local b="$1" h model desc created; h=$(backup_identity_value "$b" hostname || true); model=$(backup_identity_value "$b" model || true); desc=$(backup_identity_value "$b" openwrt_description || true); created=$(backup_identity_value "$b" backup_created || true); [ -n "$h" ] && echo "  Source hostname: $h"; [ -n "$model" ] && echo "  Source model:    $model"; [ -n "$desc" ] && echo "  OpenWrt:         $desc"; [ -n "$created" ] && echo "  Created:         $created"; }
handle_selected_backup() { local b="$1" a; echo "Selected backup: $b"; print_backup_identity_lines "$b"; [ -f "$b/$CHECKSUMS_NAME" ] || warn "This backup is missing $CHECKSUMS_NAME. Restore will fail until the file is restored."; echo; while true; do a=$(prompt_value "Action: [r]estore, [d]elete, [c]ancel" c); a=$(printf "%s" "$a"|tr A-Z a-z); case "$a" in r|restore) restore_backup "$b" "$DEFAULT_RESTORE_TARGET"; return;; d|delete) ask_yes_no "Delete selected backup permanently? [y/N]" n || return; acquire_lock; rm -rf "$b"; rmdir "$(dirname "$b")" 2>/dev/null || true; echo "Backup deleted: $b"; return;; c|cancel|q|quit|'') echo Cancelled.; return;; *) echo "Enter r, d, or c.";; esac; done; }
...
list_backups_interactive() {
    ...
    gather_host_candidates "$root" "$hostf"; [ -s "$hostf" ] || { rm -f "$hostf" "$backupf"; die "No backups found in $root. Expected layout: $root/<hostname>/<backup_dir>"; }
    select_host_candidate "$hostf" || { rm -f "$hostf" "$backupf"; return 0; }; rm -f "$hostf"; [ -n "$SELECTED_HOST_DIR" ] || { rm -f "$backupf"; return 0; }
    gather_backup_candidates "$SELECTED_HOST_DIR" "$backupf"; [ -s "$backupf" ] || { rm -f "$backupf"; die "No restorable backups found in $SELECTED_HOST_DIR"; }
    select_backup_candidate "$backupf" "$SELECTED_HOST_DIR" || { rm -f "$backupf"; return 0; }; rm -f "$backupf"; [ -n "$SELECTED_BACKUP_DIR" ] && handle_selected_backup "$SELECTED_BACKUP_DIR"
}

rotate_old_backups() { local host="$1" f="/tmp/${SCRIPT_BASENAME}.rotate.$$" n=0 b rest; [ "$RETENTION_COUNT" -eq 0 ] && return; gather_backup_candidates "$host" "$f"; while IFS='|' read -r b rest; do [ -n "$b" ] || continue; n=$((n+1)); [ "$n" -gt "$RETENTION_COUNT" ] && { rm -rf "$b"; echo "Rotated old backup: $b"; }; done < "$f"; rm -f "$f"; }
create_backup() {
    ...
    local root host target stage size fs
    ...
    mkdir -p "$host"; fs=$(df -T "$root" 2>/dev/null|awk 'NR==2{print $2}'); case "$fs" in tmpfs|ramfs) [ "$ALLOW_RAM_BACKUP" = 1 ] || die "Backup root is on RAM filesystem ($fs). Use --allow-ram to override.";; esac
    [ ! -e "$target" ] || die "Backup directory already exists: $target"; echo "Backup preflight:"; echo "  Source:      /overlay/upper"; echo "  Root:        $root"; echo "  Destination: $target"; echo "  Compressor:  $COMPRESSOR_MODE"; echo "  Retention:   $RETENTION_COUNT per hostname"; echo
...
validate_restore_inputs() { local b="$1" t; t=$(normalize_restore_target "${2:-$DEFAULT_RESTORE_TARGET}"); [ -d "$b" ] || die "Backup directory not found: $b"; [ -f "$b/$ARCHIVE_NAME" ] || die "Archive not found: $b/$ARCHIVE_NAME"; [ -d "$t" ] || die "Restore target not found: $t"; [ "$t" = / ] && die "Refusing to restore directly into /."; case "$t" in "$b"|"$b"/*) die "Restore target must not be inside the backup directory.";; esac; case "$b" in "$t"|"$t"/*) die "Restore target must not contain the backup directory.";; esac; }
confirm_restore() { local b="$1" t="$2" x; echo "Restore preflight:"; echo "  Backup: $b"; echo "  Target: $t"; print_backup_identity_lines "$b"; echo; echo "This will delete current contents of $t before extracting the backup."; echo; ask_yes_no "Continue restore? [y/N]" n || { echo "Restore cancelled."; return 1; }; x=$(prompt_value "Type RESTORE to confirm destructive restore" ""); [ "$x" = RESTORE ] || { echo "Restore cancelled."; return 1; }; }
wipe_restore_target_contents() { local t="$1" e; [ -d "$t" ] || die "Restore target not found: $t"; [ "$t" != / ] || die "Refusing to wipe / directly."; log "Clearing existing contents of $t"; for e in "$t"/* "$t"/.[!.]* "$t"/..?*; do [ -e "$e" ] || [ -L "$e" ] || continue; rm -rf "$e"; done; }
...
restore_backup() { local b="$1" t; t=$(normalize_restore_target "${2:-$DEFAULT_RESTORE_TARGET}"); require_commands tar sha256sum awk sed grep sort wc du date uname dirname basename find rm df mkdir cat; validate_restore_inputs "$b" "$t"; verify_checksums "$b"; confirm_restore "$b" "$t" || return 0; acquire_lock; wipe_restore_target_contents "$t"; log "Extracting $ARCHIVE_NAME into $t"; tar -xzf "$b/$ARCHIVE_NAME" -C "$t"; echo; echo "Overlay restore completed."; [ "$t" = "$DEFAULT_RESTORE_TARGET" ] && prompt_reboot_now || echo "Test restore target populated successfully. No reboot required."; }

reconfigure_backup_root() { local cur new root; cur=$(normalize_backup_root_path "$BACKUP_ROOT"); echo "Current backup root:"; echo "  $cur"; echo; new=$(prompt_value "Enter storage directory or full $BACKUP_ROOT_DIRNAME path" "$cur"); root=$(normalize_backup_root_path "$new"); mkdir -p "$root"; [ -w "$root" ] || die "Backup root is not writable: $root"; BACKUP_ROOT="$root"; write_config "$root"; log "Saved config to $CONFIG_PATH"; echo "New backup root: $BACKUP_ROOT"; }
